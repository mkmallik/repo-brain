...
def load_state() -> dict:
    ...
    return {"ingested": {}, "repos": {}, "query_count": 0, "last_lint": None, "total_cost": 0.0}
...
def update_state(updater) -> dict:
    """Atomic read-modify-write for state.json to avoid concurrent write races.

    Usage:
        update_state(lambda s: s["repos"].update({"coplatform": {...}}))
    """
    import fcntl
...
def read_all_wiki_content() -> str:
    ...
    # Include repo knowledge articles (recursive)
    if REPOS_DIR.exists():
        for md_file in sorted(REPOS_DIR.rglob("*.md")):
            rel = md_file.relative_to(KNOWLEDGE_DIR)
            content = md_file.read_text(encoding="utf-8")
            parts.append(f"## {rel}\n\n{content}")
...
def list_wiki_articles() -> list[Path]:
    ...
    # Include repo articles (recursive)
    if REPOS_DIR.exists():
        articles.extend(sorted(REPOS_DIR.rglob("*.md")))
    return articles
