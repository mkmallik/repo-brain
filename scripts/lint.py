...
from config import KNOWLEDGE_DIR, REPORTS_DIR, get_tracked_repos, now_iso, today_iso
from utils import (
    count_inbound_links,
    extract_wikilinks,
    file_hash,
...
def check_unscanned_repos() -> list[dict]:
    """Check for tracked repos that haven't been scanned yet."""
    state = load_state()
    repos_state = state.get("repos", {})
    issues = []
    for repo_name in get_tracked_repos():
        if repo_name not in repos_state:
            issues.append({
                "severity": "warning",
                "check": "unscanned_repo",
                "file": f"(repo: {repo_name})",
                "detail": f"Tracked repo '{repo_name}' has not been scanned. Run: uv run python scripts/scan_repo.py --repo {repo_name}",
            })
    return issues
...
def main():
    ...
    checks = [
        ...
        ("Unscanned repos", check_unscanned_repos),
    ]
