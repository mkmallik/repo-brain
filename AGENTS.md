...
### Repo Overview Articles (`knowledge/repos/{repo-name}/overview.md`)

High-level architecture and API surface of a tracked code repository. Created by the one-time repo scanner.
...
[High-level architecture, patterns, framework choices]
...
- [[repos/{repo-name}/module-x]] - Brief description
- [[repos/{repo-name}/module-y]] - Brief description

## External Dependencies

[APIs called, services integrated with, other repos consumed]
```

### Repo Module Articles (`knowledge/repos/{repo-name}/module-{name}.md`)

One per major module/service within a repo.

```markdown
---
title: "Module: {name}"
repo: "{repo-name}"
module_path: "src/path/to/module"
tags: [repo, module]
created: 2026-04-09
updated: 2026-04-09
---
...
[Important functions with signatures and brief descriptions]

## Data Models Used

- [[repos/{repo-name}/model-x]] - How it's used here
...
### Repo Data Model Articles (`knowledge/repos/{repo-name}/model-{name}.md`)

Database models, schemas, and type definitions.

```markdown
---
title: "Model: {name}"
repo: "{repo-name}"
model_path: "src/path/to/model.py"
tags: [repo, model]
created: 2026-04-09
updated: 2026-04-09
---
...
[Foreign keys, associations, references to other models]
...
[How this model is typically queried/used]
```

### Repo API Articles (`knowledge/repos/{repo-name}/api-{name}.md`)

Endpoint groups / router definitions.
...
[Auth requirements for this endpoint group]
...
### Repo Flow Articles (`knowledge/repos/{repo-name}/flow-{name}.md`)

Key end-to-end workflows that span multiple modules/services.
...
1. [Step 1] - module/service involved
2. [Step 2] - module/service involved

## Services Involved

- [[repos/{repo-name}/module-x]]
- [[repos/{repo-name}/module-y]]

## Data Transformations

[Key data shape changes through the flow]
...
### Cross-Repo Connection Articles (`knowledge/repos/connections/{name}.md`)

Patterns and relationships that span multiple repositories.

```markdown
---
title: "Cross-Repo: {name}"
connects:
  - "repos/{repo-a}/module-x"
  - "repos/{repo-b}/module-y"
tags: [repo, cross-repo]
created: 2026-04-09
updated: 2026-04-09
---
...
[What links these components across repos]

## Data Flow

[How data moves between the repos]

## Integration Points

[Specific APIs, queues, shared databases, etc.]
```
...
|   |-- connections/                 #   Cross-cutting insights linking 2+ concepts
|   |-- qa/                          #   Filed query answers (compounding knowledge)
|   |-- repos/                       #   Repository knowledge (architecture, APIs, models)
|   |   |-- {repo-name}/            #     Per-repo articles (overview, modules, models, apis, flows)
|   |   |-- connections/             #     Cross-repo connection articles
|-- scripts/                         # CLI tools
...
|   |-- repo_utils.py                #   Git helpers for repo scanning/syncing
|   |-- scan_repo.py                 #   One-time deep repo scanner (interactive)
|   |-- sync_repos.py                #   Daily git pull + incremental article updates
